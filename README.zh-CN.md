...
除了网站能力，OpenCLI 还是一个 **CLI 枢纽**：你可以把 `gh`、`docker`、`vercel`、`lark-cli`、`dws`、`wecom-cli`、`obsidian` 等本地工具统一注册到 `opencli` 下，也可以通过桌面端适配器控制 Cursor、Codex、Antigravity、ChatGPT、Notion 等 Electron 应用。
...
- **多 Profile Browser Bridge** — 在多个 Chrome profile 里安装扩展，然后通过 `--profile`、`OPENCLI_PROFILE` 或 `opencli profile use` 精确路由浏览器命令。
...
- **CLI 枢纽** — 统一发现、自动安装、纯透传外部 CLI，例如 `gh`、`docker`、`vercel`、`lark-cli`、`dws`、`wecom-cli`、`obsidian`。
...
### 5. 可选：给 Chrome profile 起别名

如果你有多个 Chrome profile，可以先列出已连接 profile，再绑定本地别名：
...
只有一个已连接 profile 时，OpenCLI 会自动使用它；多个 profile 同时在线且没有默认值时，会要求你显式选择。
...
- `opencli convention-audit [target]` 扫描 adapter 是否违反 agent-native 约定
...
`browser` 可用命令包括：`open`、`state`、`click`、`type`、`fill`、`select`、`keys`、`wait`、`get`、`find`、`extract`、`frames`、`screenshot`、`scroll`、`back`、`eval`、`network`、`console`、`dialog`、`bind`、`unbind`、`tab list`、`tab new`、`tab select`、`tab close`、`init`、`verify`、`close`。
...
- 统一代理本地二进制工具，例如 `gh`、`docker`、`vercel`、`lark-cli`、`dws`、`wecom-cli`、`obsidian`
...
| `OPENCLI_PROFILE` | — | 多个 Chrome profile 同时连接时，指定 Browser Bridge profile alias/contextId |
...
| `OPENCLI_BROWSER_REUSE` | adapter 默认值 | 设为 `none` 或 `site` 来覆盖 adapter 的浏览器 tab reuse 策略。`--reuse <none\|site>` 会设置它 |
...
`opencli browser *` 默认使用前台窗口并保留 tab lease。浏览器型 adapter 默认使用后台 automation 窗口并在命令结束后释放 one-shot tab lease；部分交互式 adapter 会默认启用 `--reuse site`，这也会保留站点 tab lease。需要一次性 tab 时可显式传 `--reuse none`。
